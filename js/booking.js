/**
 * Freeport request-to-book form (Phase 19D).
 * Server-side pricing is authoritative — client total is for review only.
 * Guest model: paying participants (ages 3+) + free children (0–2). No child discount.
 */
(function () {
  const form = document.getElementById("fpo-booking-form");
  if (!form || !window.FPO_COMMERCIAL) return;

  const cfg = window.FPO_COMMERCIAL;
  ...
  const guestModel = product.guestModel || "ages3_plus_infant";
  ...
  function refresh() {
    ...
    const infants = guestModel === "ages3_plus_infant" ? num("infants", 0) : 0;
    ...
    if (guestModel === "ages3_plus_infant") {
  ...
  function sessionId() {
    ...
    return "atg-" + Date.now() + "-" + Math.random().toString(36).slice(2, 10);
  }
  ...
  form.addEventListener("submit", async function (event) {
    ...
    const totalGuests = adults + infants;
    if (adults < 1) {
      showError("Please include at least one paying participant (age 3+).");
      return;
    }
    if (totalGuests < 1 || totalGuests > max) {
      showError("Please choose between 1 and " + max + " guests (including free children age 0–2).");
      return;
    }
    ...
    const stayBehindBox = document.getElementById("staybehind_ack");
    ...
    if (!stayBehindBox || !stayBehindBox.checked) {
      showError("Please acknowledge the stay-behind return transport terms.");
      return;
    }
