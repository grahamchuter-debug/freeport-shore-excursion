/**
 * Freeport request-to-book Worker (Classic Beach Day only).
 *
 ...
 */
import { freeportBookingCore } from "../../../shared/destinations/freeport";
...
async function handlePreviewRequest(request: Request, env: Env): Promise<Response> {
    ...
    return jsonResponse(
      {
        ...
        message:
          "Live Stripe is not enabled for this Freeport Worker. Charge-then-refund still requires an explicit production configuration. No payment was taken.",
      },
      503,
    );
  }
  ...
  const product = findFreeportBookingProduct(body.productId);
    ...
    return jsonResponse(
      {
        ...
        code: "ELIGIBILITY",
        message:
          "Please acknowledge that if you stay behind at Port Lucaya Marketplace or Lucaya Beach after the tour group departs, return transport is not included.",
      },
      400,
    );
  }
  ...
  const reference = createBookingReference(freeportBookingCore);
  ...
  const destinationBrand = destinationBrandFromCore(freeportBookingCore);
