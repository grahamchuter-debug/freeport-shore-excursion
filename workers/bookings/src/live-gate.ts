 ...
 */
/** Phase 15D: Graham-authorised live request-to-book unlock (Garden of the Groves & Freeport City Tour). */
export const LIVE_PAYMENTS_CODE_ENABLED = true;

export const LIVE_UNLOCK_PHRASE = "FREEPORT_LIVE_UNLOCK";
...
export function liveReadinessGaps(
  ...
  if (!(env.SITE_BASE_URL || "").includes("freeportshoreexcursion.com")) gaps.push("site_base_url");
...
export function liveCheckoutBlock(
  ...
  if (!LIVE_PAYMENTS_CODE_ENABLED) {
    return {
      ...
      message: "Live Stripe is not enabled for Freeport. The live code flag is off. No payment was taken.",
    };
  }
  ...
  if (!bookingsAreEnabled(env)) {
    return {
      ...
      message: "Freeport online booking is switched off. No payment was taken.",
    };
  }
  if (!env.DB) {
    return {
  ...
  if (!site.includes("freeportshoreexcursion.com")) {
    return {
      code: "SITE_BASE_URL",
      message: "Live Checkout requires SITE_BASE_URL on freeportshoreexcursion.com. No payment was taken.",
    };
  }
  if (!product) {
    return {
